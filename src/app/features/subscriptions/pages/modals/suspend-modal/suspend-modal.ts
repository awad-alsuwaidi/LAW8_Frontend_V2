...
@Component({
  ...
  templateUrl: './suspend-modal.html',
  styleUrl: './suspend-modal.scss',
})
export class SuspendModal implements OnDestroy {
  ...
  submit(): void {
    ...
    this.errorMessage = '';
    this.service
      .suspend(this.subscription.id, { reason: this.reason.trim() || undefined })
      .pipe(takeUntil(this.destroy$), finalize(() => { this.isLoading = false; this.cdr.markForCheck(); }))
      .subscribe({
        next: (updated) => this.saved.emit(updated),
        error: (err) => { this.errorMessage = err?.error?.message ?? this.t('subscriptions.suspend.failed'); },
      });
  }
